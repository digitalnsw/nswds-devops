...
| `install / install` (or the release job) red on `npm audit signatures`, e.g. `E404 ... /-/npm/v1/attestations/<pkg>@<version>` | the lockfile holds a package version whose registry signature or advertised provenance attestation cannot be verified | move the lockfile to a version that verifies: `npm update <pkg> --package-lock-only` when one is inside the parent's range; otherwise a per-repo `overrides` entry, registered in the exceptions table below. The check reads the live registry, so a lost attestation turns every open PR in the repo red at once; merge the lockfile fix first, then update the other branches. Never skip the step |
...
| nswds-email-framework, nswds-email-starter | `proxy-addr` pinned to `^2.0.8` via npm `overrides` | Critical User Impersonation (`SNYK-JS-PROXYADDR-19812342`) in `proxy-addr@2.0.7`, reached through `@maizzle/framework > express`; the only upstream fix is an override. Kept per-repo because `@maizzle/framework` exists only in these two and the fleet has no npm-`overrides` sync mechanism ([config-single-source-of-truth.md](docs/config-single-source-of-truth.md)). Transitive-only, so not a literal self-override — Renovate-safe, and `default.json` disables Renovate updates to `overrides`-typed deps anyway | `@maizzle/framework`'s Express dependency resolves `proxy-addr ≥ 2.0.8` on its own |
| nswds-email-design | `next` pinned to `16.3.6` under `@react-email/ui` via npm `overrides` | `@react-email/ui` (the dev-only `react-email` preview server) pins `next` exactly to `16.3.3`, inside the CVE-2026-94545 range (`>=16.2.0 <16.3.6`); up to `@react-email/ui@6.11.0` there is no upstream release outside it, so the only fix is an override. Scoped under `@react-email/ui` and the root has no direct `next` dependency, so it is not a literal self-override. `default.json` disables Renovate updates to `overrides`-typed deps, so the pin does not follow the apps' `next` and can hold `@react-email/ui` below a newer pin it ships | `@react-email/ui` depends on `next ≥ 16.3.6` on its own; then remove the override |
