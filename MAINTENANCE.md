...
| Snyk policy canary: a repo reports `manual` and keeps the issue open | manual repos are deliberately reported until converted — none are marked manual today (dtl-sandbox, the last one, was converted 2026-09-04) | convert the repo by hand and delete its directive from `snyk-policy/repos.json` |
